...
Процедура УстановитьУровниУлицыПоЧастямАдреса(ИдентификаторНаселенногоПункта, ЧастиАдреса) Экспорт
	...
	Для Индекс = 0 По ЧастиАдреса.Количество() -1 Цикл
		...
		СтрокаИз = СтрокаИз + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон, НазваниеТаблицы,
			СтрЗаменить(ЧастьАдреса.Наименование, """",""""""),
			СтрЗаменить(ЧастьАдреса.Сокращение, """",""""""));
...
Функция КодыАдресаИКодыКЛАДР(Знач Адрес, Источник = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(Результат.КодыАдреса.Идентификатор) Тогда
		Результат.КодыАдреса.Идентификатор = Результат.ИдентификаторАдресногоОбъекта;
	КонецЕсли;
...
Функция АнализОдногоАдресаПоКлассификатору(Адрес, Уровни)
	...
	Для Каждого Запись Из ЗаписиДомов Цикл;
		...
		ИначеЕсли ПроверятьНаселенныйПункт И Запись.НаселенныйПунктНайден <> Истина Тогда
			ПроверитьИсторию = Истина;
		КонецЕсли;
		...
		ИначеЕсли ПроверятьДополнительный И Запись.ДополнительныйНайден <> Истина Тогда
			ПроверитьИсторию = Истина;
		КонецЕсли;
	...
	Если ПроверитьИсторию Тогда
		НовыйАдрес = АнализИсторииОдногоАдресаПоКлассификатору(Адрес, Уровни);
		Если НовыйАдрес <> Неопределено Тогда
			
			Результат.Вставить("Исторический", Истина);
...
Процедура ТекстОпциональныхПолейУпорядочиванияЗапросаПроверкиАдресаИстория(ТекстЗапроса, Флаг, ИмяПоля)
	Если Флаг Тогда
		ТекстЗапроса = ТекстЗапроса + ", " + Символы.ПС + ИмяПоля + ".ОкончаниеДействияЗаписи УБЫВ";
	КонецЕсли;
КонецПроцедуры

Функция ТекстОпциональныхПолейЗапросаПроверкиАдресаИстория(ФлагПроверки, ИмяПоля)
	
	Если ФлагПроверки Тогда
		Вариант = "
			|	ВЫБОР КОГДА [Поле].Идентификатор ЕСТЬ NULL ТОГДА ЛОЖЬ ИНАЧЕ ИСТИНА КОНЕЦ КАК [Поле]Найден,
			|	[Поле].АдресныйОбъект                КАК [Поле]АдресныйОбъект,
...
// Локальная проверка одного адреса.
//
Функция АнализИсторииОдногоАдресаПоКлассификатору(Адрес, Уровни)
	...
	КодСубъектРФ = АдресныйКлассификатор.КодРегионаПоНаименованию(Адрес.СубъектРФ);
	...
	ТекстУпорядочить = "УПОРЯДОЧИТЬ ПО СубъектРФ.ОкончаниеДействияЗаписи УБЫВ";
	ТекстОпциональныхПолейУпорядочиванияЗапросаПроверкиАдресаИстория(ТекстУпорядочить, ПроверятьОкруг,                "Округ");
	ТекстОпциональныхПолейУпорядочиванияЗапросаПроверкиАдресаИстория(ТекстУпорядочить, ПроверятьРайон,                "Район");
	ТекстОпциональныхПолейУпорядочиванияЗапросаПроверкиАдресаИстория(ТекстУпорядочить, ПроверятьГород,                "Город");
	ТекстОпциональныхПолейУпорядочиванияЗапросаПроверкиАдресаИстория(ТекстУпорядочить, ПроверятьВнутригородскойРайон, "ВнутригородскойРайон");
	ТекстОпциональныхПолейУпорядочиванияЗапросаПроверкиАдресаИстория(ТекстУпорядочить, ПроверятьНаселенныйПункт,      "НаселенныйПункт");
	ТекстОпциональныхПолейУпорядочиванияЗапросаПроверкиАдресаИстория(ТекстУпорядочить, ПроверятьУлицу,                "Улица");
	ТекстОпциональныхПолейУпорядочиванияЗапросаПроверкиАдресаИстория(ТекстУпорядочить, ПроверятьДополнительный,       "Дополнительный");
	ТекстОпциональныхПолейУпорядочиванияЗапросаПроверкиАдресаИстория(ТекстУпорядочить, ПроверятьПодчиненный,          "Подчиненный");
	...
	Запрос.Текст = "ВЫБРАТЬ
		...
		|	СубъектРФ.Дополнительно                 КАК СубъектРФДополнительно,
		|
		|	" + ТекстОпциональныхПолейЗапросаПроверкиАдресаИстория(ПроверятьОкруг,                "Округ") + "
		|	" + ТекстОпциональныхПолейЗапросаПроверкиАдресаИстория(ПроверятьРайон,                "Район") + "
		|	" + ТекстОпциональныхПолейЗапросаПроверкиАдресаИстория(ПроверятьГород,                "Город") + "
		|	" + ТекстОпциональныхПолейЗапросаПроверкиАдресаИстория(ПроверятьВнутригородскойРайон, "ВнутригородскойРайон") + "
		|	" + ТекстОпциональныхПолейЗапросаПроверкиАдресаИстория(ПроверятьНаселенныйПункт,      "НаселенныйПункт") + "
		|	" + ТекстОпциональныхПолейЗапросаПроверкиАдресаИстория(ПроверятьУлицу,                "Улица") + "
		|	" + ТекстОпциональныхПолейЗапросаПроверкиАдресаИстория(ПроверятьДополнительный,       "Дополнительный") + "
		|	" + ТекстОпциональныхПолейЗапросаПроверкиАдресаИстория(ПроверятьПодчиненный,          "Подчиненный") + "
		|
		|	NULL                              КАК ДомаКодКЛАДР
		|
		|ИЗ
		|	// Иерархия подчиненности
		|	РегистрСведений.ИсторияАдресныхОбъектов КАК СубъектРФ
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	РегистрСведений.ДополнительныеАдресныеСведения КАК СубъектРФДополнительно
		|ПО
		|	СубъектРФДополнительно.Идентификатор = СубъектРФ.Дополнительно
		...
		|" + ?(ПроверятьРайон, "
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	РегистрСведений.ИсторияАдресныхОбъектов КАК Район
		|ПО
		|	  Район.Наименование = &РайонНаименование
		|	И Район.Сокращение   = &РайонСокращение
		|	И Район.Уровень                    = 3
		...
		|" + ?(ПроверятьГород, " 
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	РегистрСведений.ИсторияАдресныхОбъектов КАК Город
		|ПО
		|	  Город.Наименование = &ГородНаименование
		|	И Город.Сокращение   = &ГородСокращение
		|	И Город.Уровень                    = 4
		...
		|" + ?(ПроверятьВнутригородскойРайон, "
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	РегистрСведений.ИсторияАдресныхОбъектов КАК ВнутригородскойРайон
		|ПО
		|	  ВнутригородскойРайон.Наименование = &ВнутригородскойРайонНаименование
		|	И ВнутригородскойРайон.Сокращение   = &ВнутригородскойРайонСокращение
		|	И ВнутригородскойРайон.Уровень                    = 5
		...
		|" + ?(ПроверятьНаселенныйПункт, "
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	РегистрСведений.ИсторияАдресныхОбъектов КАК НаселенныйПункт
		|ПО
		|	  НаселенныйПункт.Наименование = &НаселенныйПунктНаименование
		|	И НаселенныйПункт.Сокращение   = &НаселенныйПунктСокращение
		|	И НаселенныйПункт.Уровень                    = 6
		...
		|" + ?(ПроверятьУлицу, "
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	РегистрСведений.ИсторияАдресныхОбъектов КАК Улица
		|ПО
		|	  Улица.Наименование = &УлицаНаименование
		|	И Улица.Сокращение   = &УлицаСокращение
		|	И Улица.Уровень                    = 7
		...
		|" + ?(ПроверятьДополнительный, "
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	РегистрСведений.ИсторияАдресныхОбъектов КАК Дополнительный
		|ПО
		|	  Дополнительный.Наименование = &ДополнительныйНаименование
		|	И Дополнительный.Сокращение   = &ДополнительныйСокращение
		|	И Дополнительный.Уровень                    = 65
		|	И Дополнительный.КодСубъектаРФ              = СубъектРФ.КодСубъектаРФ
		|	И Дополнительный.КодОкруга                  = " + ?(ПроверятьОкруг, "Округ.КодОкруга", "0") + "
		|	И Дополнительный.КодРайона                  = " + ?(ПроверятьРайон, "Район.КодРайона", "0") + "
		|	И Дополнительный.КодГорода                  = " + ?(ПроверятьГород, "Город.КодГорода", "0") + "
		...
		|" + ?(ПроверятьПодчиненный, "
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	РегистрСведений.ИсторияАдресныхОбъектов КАК Подчиненный
		|ПО
		|	  Подчиненный.Наименование = &ПодчиненныйНаименование
		|	И Подчиненный.Сокращение   = &ПодчиненныйСокращение
		|	И Подчиненный.Уровень                    = 7
		|	И Подчиненный.КодСубъектаРФ              = СубъектРФ.КодСубъектаРФ
		|	И Подчиненный.КодОкруга                  = " + ?(ПроверятьОкруг, "Округ.КодОкруга", "0") + "
		|	И Подчиненный.КодРайона                  = " + ?(ПроверятьРайон, "Район.КодРайона", "0") + "
		|	И Подчиненный.КодГорода                  = " + ?(ПроверятьГород, "Город.КодГорода", "0") + "
		...
		|// Условие на регион, который есть всегда
		|ГДЕ
		|	СубъектРФ.КодСубъектаРФ              = &КодСубъектРФ
		|	И СубъектРФ.КодОкруга                  = 0
		|	И СубъектРФ.КодРайона                  = 0
		|	И СубъектРФ.КодГорода                  = 0
		|	И СубъектРФ.КодВнутригородскогоРайона  = 0
	...
	Запрос.УстановитьПараметр("КодСубъектРФ", КодСубъектРФ);
	...
	УлицаНайдена = Ложь;
	ВариантАдреса = Новый Структура;
	История = Новый Соответствие;
	Для Каждого Запись Из ЗаписиДомов Цикл;
		ЗаполнитьПочтовыйИндексИКодКЛАДРПоИерархииФИАС(ВариантАдреса, Запись, Ложь);
		...
		Если ОшибкиПроверки.Количество() = 0 Тогда
			...
			Если ПроверятьДополнительный Тогда
				АдресНовый = АктуальныеАдресныеСведения(Запись.ДополнительныйАдресныйОбъект);
				Возврат АдресНовый;
			КонецЕсли;
			
			Если ПроверятьУлицу И Запись.УлицаНайден Тогда
				УлицаНайдена = Истина;
				АдресНовый = АктуальныеАдресныеСведения(Запись.УлицаАдресныйОбъект);
				Возврат АдресНовый;
			КонецЕсли;
			
			// Вся запись правильная
			КорректныеЗаписи.Добавить(АдресНовый);
		КонецЕсли;
	КонецЦикла;
...
Функция СформироватьОписаниеЗдания(НазваниеВладения, НомерВладения, НомерКорпуса, НазваниеСтроения, НомерСтроения, ДополнительныеПараметры = Неопределено)
	...
	Если ВыделятьСтрокуПоиска Тогда
		Дом = Строка(НазваниеВладения) + Новый ФорматированнаяСтрока(НазваниеВладения + " ", 
	...
	Иначе
		Результат = Строка(НазваниеВладения) + " " + НомерВладения + КорпусИСтроение;
	КонецЕсли;
...
Процедура ЗаполнитьПочтовыйИндексИКодКЛАДРПоИерархииФИАС(Вариант, ЗаписьИсточника, ПроверятьДома = Истина)
	
	УстановитьРеквизитЗаписиФИАСИерархически(Вариант, "Индекс",  ЗаписьИсточника, "ПочтовыйИндекс", ПроверятьДома);
	УстановитьРеквизитЗаписиФИАСИерархически(Вариант, "КодКЛАДР", ЗаписьИсточника, "КодКЛАДР", ПроверятьДома);
...
Процедура УстановитьРеквизитЗаписиФИАСИерархически(Приемник, ИмяПоляПриемника, Источник, ИмяПоляИсточника, ПроверятьДома = Истина)
	
	Варианты = Новый Массив;
	Если ПроверятьДома И ИмяПоляИсточника <> "Идентификатор" Тогда
