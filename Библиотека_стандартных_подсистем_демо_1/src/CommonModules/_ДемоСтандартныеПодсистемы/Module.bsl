...
// Выполняет проверку проведенности документа _ДемоСчетФактураПолученный, выданного по двум контрагентам.
// Демонстрирует работу с несколькими видами проверок и их свойствами.
//
// Параметры:
//   Проверка            - СправочникСсылка.ПравилаПроверкиУчета - Выполняемая проверка.
//   ПараметрыПроверки   - Структура - Параметры выполняемой проверки:
//     * Идентификатор      - Строка - Строковый идентификатор проверки.
//     * ДатаНачалаПроверки - Дата - Пороговая дата, обозначающая границу проверяемых
//                            объектов (только для объектов с датой). Объекты, дата которых меньше 
//                            указанной, не следует проверять. По умолчанию не заполнено (т.е. проверять все).
...
Процедура ПроверитьПроведенностьСчетаФактурыПолученного(Проверка, ПараметрыПроверки) Экспорт
	...
	|ГДЕ
	|	&Условие И НЕ _ДемоСчетФактураПолученный.Проведен
	...
	Если Контрагенты.Количество() > 0 Тогда
		ТекстЗапроса = СтрЗаменить(ШаблонТекстаЗапроса, "&Условие", "_ДемоСчетФактураПолученный.Контрагент В (&Контрагенты)");
		ПараметрыЗапроса.Вставить("Контрагенты", Контрагенты);
	Иначе
		ТекстЗапроса = СтрЗаменить(ШаблонТекстаЗапроса, "&Условие", "ИСТИНА");
	КонецЕсли;	
	...
	Пока Результат.Следующий() Цикл
		...
		Проблема = КонтрольВеденияУчета.ОписаниеПроблемы(Результат.ПроблемныйОбъект, ПараметрыПроверки);
		Проблема.ВидПроверки = КонтрольВеденияУчета.ВидПроверки(
			КонтрольВеденияУчета.ПараметрыВыполненияПроверки("_ДемоПроверкиВеденияУчета", Результат.Контрагент));
		Проблема.УточнениеПроблемы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'По контрагенту ""%1"" обнаружен непроведенный документ ""%2""'"),
			Результат.Контрагент, Результат.ПроблемныйОбъект);
