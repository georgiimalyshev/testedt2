...
Процедура ЗаполнитьОшибкиАдресаПоКлассификатору(XDTOАдресРФ, ФлагиПроверки, Результат)
	...
	Для Каждого РезультатПроверки Из РезультатыАнализа.Данные Цикл
		...
		Если РезультатПроверки.Свойство("Исторический") И  РезультатПроверки.Исторический Тогда
			
			Если РезультатПроверки.Варианты.Количество() > 0 И РезультатПроверки.Варианты[0].Свойство("Данные") Тогда
				
				ПредставлениеАдреса = ПредставлениеАдреса(Новый Структура("ФорматАдреса", "ФИАС"), РезультатПроверки.Варианты[0].Данные, "Россия");
				
				ОписаниеОшибки = Новый Структура("ПутьXPath, СущностьПоля, АдресПроверен", "Представление", "", РезультатПроверки.АдресПроверен);
				ШаблонОшибки = НСтр("ru='Адрес некорректный, т.к. он устарел
					|Актуальный: %1'");
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОшибки, ПредставлениеАдреса);
