...
Процедура ВыполнитьДействияПоНастройкеНовогоОбменаДанными(Отказ,
	...
	ВыполнитьДействияПоНастройкеОбменаДанными(СтатусЗавершения,
		НастройкаОтборовНаУзле, ЗначенияПоУмолчаниюНаУзле, КодЭтогоУзла, КодНовогоУзла, РегистрироватьДанныеДляВыгрузки, ИспользоватьНастройкиТранспорта);
		
	Отказ = СтатусЗавершения.Отказ;
...
//
Процедура ВебСервисВыполнитьДействияПоНастройкеНовогоОбменаДанными(Отказ,
		НастройкаОтборовНаУзле, ЗначенияПоУмолчаниюНаУзле, СообщениеОбОшибке = "") Экспорт
	...
	Если ОбменДаннымиСервер.ЕстьАлгоритмМенеджераПланаОбмена("ПриПолученииДанныхОтправителя",ИмяПланаОбмена) Тогда
		...
		Исключение
			...
			СообщениеОбОшибке = КраткоеПредставлениеОшибки(Инфо);
			СообщитьИнформациюОбОшибке(Инфо, Отказ);
			Возврат;
		КонецПопытки;
	КонецЕсли;
...
Процедура ВыполнитьДействияПоНастройкеОбменаДанными(
	...
	Исключение
		...
		Инфо = ИнформацияОбОшибке();
		СтатусЗавершения.СообщениеОбОшибке = КраткоеПредставлениеОшибки(Инфо);
		СообщитьИнформациюОбОшибке(Инфо, СтатусЗавершения.Отказ);
		Возврат;
	КонецПопытки;
	...
	Попытка
		...
		Если РегистрироватьДанныеДляВыгрузки
			...
			// Выполняем регистрацию изменений на узле плана обмена.
			ЗарегистрироватьИзмененияДляОбмена(СтатусЗавершения.Отказ);
		...
		Иначе
			Если ВариантРаботыМастера = "ПродолжитьНастройкуОбменаДанными" Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					...
					НаименованиеВторойБазы, КодНовогоУзлаВторойБазы);
			Иначе
