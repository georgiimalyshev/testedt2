...
Процедура ЗаполнитьШаблонПослеВыбора(Результат)
	...
	Иначе
		...
		Для каждого ИзображенияВТекстеПисьма Из Результат.Вложения Цикл
			
			Если ЗначениеЗаполнено(ИзображенияВТекстеПисьма.Идентификатор) Тогда
				Изображение = Новый Картинка(ПолучитьИзВременногоХранилища(ИзображенияВТекстеПисьма.АдресВоВременномХранилище));
				СтруктураВложений.Вставить(ИзображенияВТекстеПисьма.Представление, Изображение);
